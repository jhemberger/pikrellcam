...
class SDL_Pi_HDC1080:
	def __init__(self, twi=1, addr=HDC1080_ADDRESS):
		self.HDC1080_fr = io.open("/dev/i2c-" + str(twi), "rb", buffering=0)
		self.HDC1080_fw = io.open("/dev/i2c-" + str(twi), "wb", buffering=0)
		# set device address
		fcntl.ioctl(self.HDC1080_fr, I2C_SLAVE, addr)
		fcntl.ioctl(self.HDC1080_fw, I2C_SLAVE, addr)
		time.sleep(0.015)  # 15ms startup time
		config = HDC1080_CONFIG_ACQUISITION_MODE
		s = [HDC1080_CONFIGURATION_REGISTER, config >> 8, 0x00]
		s2 = bytearray(s)
		self.HDC1080_fw.write(s2)  # sending config register bytes
		time.sleep(0.015)  # From the data sheet

	def readTemperature(self):
		s = [HDC1080_TEMPERATURE_REGISTER]  # temp
		s2 = bytearray(s)
		self.HDC1080_fw.write(s2)
		time.sleep(0.0625)  # From the data sheet
		data = self.HDC1080_fr.read(2)  # read 2 byte temperature data
	...
	def readHumidity(self):
		...
		self.HDC1080_fw.write(s2)
		time.sleep(0.0625)  # From the data sheet
		data = self.HDC1080_fr.read(2)  # read 2 byte humidity data
		buf = array.array('B', data)
	...
	def readConfigRegister(self):
		...
		self.HDC1080_fw.write(s2)
		time.sleep(0.0625)  # From the data sheet
		data = self.HDC1080_fr.read(2)  # read 2 byte config data
		buf = array.array('B', data)
