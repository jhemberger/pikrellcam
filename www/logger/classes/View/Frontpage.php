...
abstract class Frontpage extends Base {
	...
	public function mainpage() {
		...
		$oTemplate->add('FormattedTemp', Formater::formatFloat($oCurrent->Temperature, 2));
		$oTemplate->add('FormattedHumd', Formater::formatFloat($oCurrent->Humidity, 2));
	...
	public function tables() {
		...
		foreach ($aHistory as $iIndex => $oReadout) {
			...
			$sTable .= '<td>'.Formater::formatDateTime($oReadout['Timestamp']).'</td>';
		...
		foreach ($aHistory as $iIndex => $oReadout) {
			$sTable .= '<tr>';
			$sTable .= '<td>'.Formater::formatDate($oReadout['Timestamp']).'</td>';
	...
	public function chartHead() {
		...
		foreach ($aHistory as $iIndex => $oReadout) {
			$oChartHourTemperature->push('Hour', Formater::formatTime($oReadout['Timestamp']));
			...
			$oChartHourHumidity->push('Hour', Formater::formatTime($oReadout['Timestamp']));
		...
		foreach ($aHistory as $iIndex => $oReadout) {
			$oChartDailyTemperature->push('Day', Formater::formatDate($oReadout['Timestamp']));
			...
			$oChartDailyHumidity->push('Day', Formater::formatDate($oReadout['Timestamp']));
