...
/**
 * @brief Base class of models used in the solution
 * @author Paweł
 */
abstract class Base implements \Interfaces\Model {

    /**
     * An array that stores a list of entries in the cache with a given model object
     * @var array
     */
    protected $aAssociatedCacheEntries = array();

    /**
     * Is the model supposed to use cache
     * @var boolean
     */
    ...
    /**
     * The default sorting direction when downloading the entire registry
     * @var string
     */
    protected $getAllSorting = null;

    /**
     * The method cleans the cache associated with the model
     * @param int $iId
     */
    ...
    /**
     * Setting additional conditions for the model, used for registers
     */
    ...
    public function checkIfExists($sKey, $mValue = null, $iId = null, $getId = false) {
        ...
        if (!empty($value)) {
            if (!empty($iId)) {
                return ($value->{$this->registryIdField} == $iId);
            }
            if ($getId) {
